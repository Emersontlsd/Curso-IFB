...
class IE {
  // 05 // Declara a classe IE (Inscrição Estadual)
  ...
  setNumero(numero) {
    // 09 // Método para atribuir o número da IE
    if (numero) {
      // 10 // Validação: número não pode ser vazio
  ...
  getNumero() {
    // 15 // Retorna o número da IE
    return this.#numero; // 16
  }
  setEstado(estado) {
    // 17 // Método para atribuir o estado
    if (estado) {
      // 18 // Validação: estado não pode ser vazio
  ...
  setDataRegistro(dataregistro) {
    // 25 // Define a data de registro (sem verificação de tipo aqui)
    if (dataregistro) {
      // 26
  ...
  setPJ(pj) {
    // 34 // Associa um objeto PJ à IE
    if (pj) {
      // 35 // Validação: objeto não pode ser vazio
      this.#pj = pj; // 36
      return true; // 37
    } else {
      // 38
