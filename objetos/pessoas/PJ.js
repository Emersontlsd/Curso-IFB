...
class PJ extends Pessoa {
  // Declaração da classe PJ herdando de Pessoa
  ...
  setCNPJ(cnpj) {
    ...
    */
    ...
    if (cnpj) {
      // 11
      if (cnpj.length < 18) {
        // Verifica se o CNPJ possui o tamanho mínimo adequado
  ...
  setEmail(email) {
    ...
    */
    ...
    if (email !== '') {
      // 19
      if (email.includes('@')) {
        // Verifica se o email contém "@" como validação básica
  ...
  setIE(ie) {
    // 23
    if (ie instanceof IE) {
      // Verifica se o parâmetro é instância da classe IE
      this.#ie = ie;
